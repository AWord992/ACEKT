...
def cal_loss(model, ys, r, rshft, sm, preloss=[]):
    ...
    if model_name in ["atdkt", "simplekt", "bakt_time", "sparsekt", "acekt"]:
...
def train_model(model, train_loader, valid_loader, num_epochs, opt, ckpt_path, test_loader=None, test_window_loader=None, save_model=False, data_config=None, fold=None):
    ...
    for i in range(1, num_epochs + 1):
        ...
        if timing:
            ...
            print(f"Epoch {i} cost time: {elapsed_time:.4f}s")
        if model.model_name=='lpkt':
        ...
        if i - best_epoch >= 10:
            break
    if timing:
        print(f"Average time per epoch: {np.mean(epoch_times):.4f}s")
        # 计算第一个epoch到best_epoch的时间总和
        if best_epoch > 0 and best_epoch <= len(epoch_times):
            time_to_best = sum(epoch_times[:best_epoch])
            print(f"Total time from epoch 1 to best epoch ({best_epoch}): {time_to_best:.4f}s")
        else:
            print("Warning: Cannot calculate time to best epoch - invalid best_epoch value")
